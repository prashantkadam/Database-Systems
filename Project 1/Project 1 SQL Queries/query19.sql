...
select avg(finaltbl.total)
from 
(
--select s.B#, s.FIRSTNAME,nvl(temp.total_credits,0) as total 
  select temp.total_credits as total
from students s
                join (
                        select t.B#,sum(c.CREDITS) as total_credits from
                        COURSE_CREDIT c,
                        (
                              select B#,c.COURSE#
                              from enrollments e join classes c 
                              on e.classid = c.classid and e.lgrade is  not null) t
                              where c.COURSE# = t.course#
                              GROUP by t.B#--,c.COURSE#
                            --order by B#
                       ) temp

on temp.B# =s.B#) finaltbl